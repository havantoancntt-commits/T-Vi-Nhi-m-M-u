...
const Divination: React.FC = () => {
    ...
    const handleButtonClick = async () => {
        ...
        setTimeout(async () => {
            try {
                const res = await getDivinationStick();
                setResult(res);
            } catch (err) {
    ...
    const getStickNameColor = (name: string) => {
        if (name.includes('Thượng') || name.includes('Đại Cát')) return 'text-green-400';
        if (name.includes('Hạ')) return 'text-red-400';
            ...
            {!result && (
                <Card>
                    <div className="p-6 text-center flex flex-col items-center">
                        <h2 className="text-3xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-amber-300 to-yellow-400">Thành Tâm Xin Quẻ</h2>
                        <p className="text-gray-400 mb-8 max-w-xl">Hãy giữ cho tâm trí thanh tịnh, tập trung vào điều bạn đang thắc mắc và thành tâm gieo quẻ để nhận được lời chỉ dẫn.</p>
                        ...
                        <button onClick={handleButtonClick} disabled={isLoading} className="btn-shine bg-gradient-to-r from-amber-400 to-yellow-500 text-gray-900 font-bold py-3 px-8 rounded-full shadow-lg hover:shadow-amber-400/50 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 mx-auto">
                             {isLoading ? (
                                <>
                                 ...
                                 <><YinYangIcon className="w-6 h-6" /> Thành Tâm Gieo Quẻ</>
                             )}
                        </button>
                    </div>
                </Card>
                 ...
                 <div className="mt-10 space-y-8 opacity-0 animate-fade-in-up">
                    <Card>
                        <div className="p-6 text-center">
                            <p className="text-amber-300">Quẻ Xăm Số</p>
                    ...
                    <Card>
                        <div className="text-center text-gray-300 space-y-4">
                            ...
                            <h3 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-300 to-yellow-400">Gieo Duyên Lành, Phúc Báo Viên Mãn</h3>
                            <p className="max-w-2xl mx-auto">Quẻ xăm này là một lời chỉ dẫn. Nếu quẻ này mang lại cho bạn sự sáng tỏ, hãy gieo một duyên lành để phúc báo được viên mãn và lan tỏa giá trị thiện lành đến với cộng đồng.</p>
                    ...
                    <div className="text-center mt-8">
                         <button onClick={() => setResult(null)} className="btn-shine bg-gradient-to-r from-indigo-500 to-blue-600 text-white font-bold py-3 px-8 rounded-full shadow-lg hover:shadow-indigo-500/50 transition-all duration-300 transform hover:scale-105 flex items-center gap-2 mx-auto">
                           <YinYangIcon className="w-6 h-6"/> Gieo Quẻ Mới
                        </button>
                    </div>
